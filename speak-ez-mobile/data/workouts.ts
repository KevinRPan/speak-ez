 ...
 */
export function randomizeWarmups(workoutExercises: WorkoutExercise[]): WorkoutExercise[] {
...
export function getWorkoutsByDuration(minutes: number): WorkoutTemplate[] {
  return workoutTemplates.filter(w => w.duration <= minutes);
}

export function calculateWorkoutDuration(exs: WorkoutExercise[]): number {
  let total = 0;
  exs.forEach((ex, i) => {
    total += ex.duration * ex.sets;
    total += ex.rest * (ex.sets - 1);
    if (i < exs.length - 1) {
      total += ex.rest || 30;
    }
  });
